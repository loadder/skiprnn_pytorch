...
class Scoring(nn.Module):
    def __init__(self, feature_size):
        ...
        conv_input = 512
        hidden_size = 256
        #self.scale1 = conv_lstm(hidden_size, 2, 1, 64, conv_input)
        self.scale2 = conv_lstm(hidden_size, 4, 2, 64, conv_input)
        self.scale3 = conv_lstm(hidden_size, 8, 4, 64, conv_input)
        self.attn = selfAttn(conv_input, 64, 30)
        self.lstm = nn.LSTM(input_size=conv_input, hidden_size=hidden_size, num_layers=1, batch_first=True)
        #self.linear_skip1 = nn.Linear(hidden_size, 128)
        ...
        #self.linear_merge = nn.Linear(128, 128)
        self.linear_merge2 = nn.Linear(64, 32)
    ...
    def forward(self, model_input):
        ...
        #attn, _, _ = split_rnn_outputs('skip_lstm', attn)
        attn = attn[:, -1, :]
        #output = torch.cat([self.relu(self.linear_skip2(self.scale2(model_input))), self.relu(self.linear_skip3(self.scale3(model_input)))], 1)
        output = self.relu(self.linear_skip2(self.scale2(model_input))) + self.relu(self.linear_skip3(self.scale3(model_input)))
        #output = torch.cat([output, self.relu(self.linear_attn(attn))], 1)
        output = output + self.relu(self.linear_attn(attn))
        #output = self.scale1(model_input) + self.scale2(model_input) + self.scale3(model_input) 
        #output = self.relu(self.linear_merge2(self.relu(self.linear_merge(output))))
        output = self.relu(self.linear_merge2(output))
    ...
    def loss(self, regression, actuals):
        ...
        """
        regr_loss_fn = train_mse() if self.training else val_mse()
